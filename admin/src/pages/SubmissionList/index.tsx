...
import { FormCollectionType, FormResponse, FormType, SubmissionCollectionType, SubmissionsResponse } from '../../utils/types';
...
const SubmissionList = () => {
	...
	useEffect(() => {
		if (!id) {
			submissionRequests.getSubmissions(query).then((response: SubmissionsResponse) => {
			...
			<Grid>
				<GridItem col={10} s={12}>
					<Box paddingLeft={10} paddingRight={10} paddingBottom={10}>
						{submissions.length > 0 ? <SubmissionTable submissions={submissions} form={form} /> : <></>}
					</Box>
				</GridItem>
			</Grid>
		</>
