 ...
 */
import { Button, Dialog, DialogBody, DialogFooter, Flex, LinkButton, Stack, Tr, Td, Typography } from '@strapi/design-system';
import { Pencil, ExclamationMarkCircle, Trash, Attachment, Mail } from '@strapi/icons';
            ...
            <Td>
                <Flex justifyContent="right" alignItems="right">
                    <Stack horizontal={true} spacing={3}>
                        <LinkButton to={`/plugins/${pluginId}/form/submissions/${item.id}`} startIcon={<Mail />}>
                            {formatMessage({ id: `${pluginId}.submissions.label` })}
                        </LinkButton>
                        <LinkButton to={`/plugins/${pluginId}/form/edit/${item.id}`} startIcon={<Pencil />}>
                            {formatMessage({ id: `${pluginId}.actions.edit` })}
                        </LinkButton>
                        <LinkButton to={`/plugins/${pluginId}/form/handlers/${item.id}`} startIcon={<Attachment />}>
                            {formatMessage({ id: `${pluginId}.actions.handlers` })}
                        </LinkButton>
                        <Button onClick={() => setIsVisible(true)} startIcon={<Trash />} variant="danger">
                        ...
                        >
                            ...
                            <DialogFooter
                                ...
                                endAction={
                                    <Button onClick={() => remove()} variant="danger-light" startIcon={<Trash />}>
